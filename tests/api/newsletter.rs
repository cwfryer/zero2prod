use crate::helpers::{when_sending_an_email, assert_is_redirect_to, spawn_app, ConfirmationLinks, TestApp};
...
async fn you_must_be_logged_in_to_publish_a_newsletter() {
    ...
    let response = app.post_publish_newsletters(&newsletter_body).await;
    // Assert
    assert_is_redirect_to(&response, "/login");
}
...
async fn newsletter_creation_is_idempotent() {
    ...
    let newsletter_request_body = serde_json::json!({
        ...
        "text_content": "Newsletter body as plain text.",
        "html_content": "<p>Newsletter body as HTML.</p>",
        // Idempotency key should be part of the form data
    ...
    assert!(html_page.contains(r#"<p><i>The newsletter issue has been published!</i></p>"#));
    app.dispatch_all_pending_emails().await;
    // Mock verifies on drop that we have sent the email **once**
}
